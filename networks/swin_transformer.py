...
class SwinTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        self.hw_attention = hw_attention
    ...
    def forward(self, x):
        ...
        if self.hw_attention:
            return [[parsing_result, fusion_result], [edge_result], [fea_h1, fea_w1]]
...
def swin_small_patch4_window7_224(img_size=224, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
    ...
    model = SwinTransformer(pretrain_img_size=img_size, patch_size=4, window_size=7, embed_dims=96,
                            depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24), context_in_channel=768,
                            fusion_in_channel=832, drop_path_rate=drop_path_rate, drop_rate=drop_rate,
                            attn_drop_rate=attn_drop_rate, **kwargs)
    # print(kwargs)
    settings = pretrained_settings['swin']['imagenet']
    initialize_pretrained_model(model, settings, **kwargs_bak)
...
def swin_tiny_patch4_window7_224(img_size=224, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0., **kwargs):
    ...
    model = SwinTransformer(pretrain_img_size=img_size, patch_size=4, window_size=7, embed_dims=96, depths=(2, 2, 6, 2),
                            num_heads=(3, 6, 12, 24), context_in_channel=768, fusion_in_channel=832,
                            drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, **kwargs)
    # print(kwargs)
    settings = pretrained_settings['swin']['imagenet']
    initialize_pretrained_model(model, settings, **kwargs_bak)
    return model
